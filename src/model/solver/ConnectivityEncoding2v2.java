 ...
 * not finish
 * 12/11/2015
 * Connectivity Encoding use Zones and patterns before encode Plus
 * @author DoDuy
 */
public final class ConnectivityEncoding2v2 extends ASolverSATE{
    private final int flag[][];
    private final boolean blackBound[][];
    private final boolean isBoundOfblackBound[][];
    ...
    private int[][] zones;
    private int[][][][] path;
    public ConnectivityEncoding2v2(int n, int m, int b[][]){
        super(n,m,b);
        flag = new int[Rows][Columns];
        blackBound = new boolean[Rows][Columns];
        isBoundOfblackBound = new boolean[Rows][Columns];
        int j;
        for(int i = 0;i < Rows ; i++)
            for(j = 0;j < Columns ; j++)
            {
                flag[i][j] = -1;
                blackBound[i][j] = false;
                isBoundOfblackBound[i][j] = false;
            }
    }
    ...
    protected void encodeVar(){
        ...
        for(i = 0; i < Rows; i++)                       
            for(j = 0; j < Columns; j++)
            {
                ...
                blackBound[i][j] = flag[i][j] == 1 && ((i-1>=0 && j-1>=0 && flag[i-1][j-1] == -1)
                                                     ||(i-1>=0 && j+1<Columns && flag[i-1][j+1] == -1)
                                                     ||(i+1<Rows && j-1>=0 && flag[i+1][j-1] == -1)
                                                     ||(i+1<Rows && j+1<Columns && flag[i+1][j+1] == -1));
            }
    ...
    private void findZone(int i, int j, int zoneNumber){
        ...
        if(i+1 < Rows && j+1 < Columns && (notFix[i+1][j+1] || blackBound[i+1][j+1]) && zones[i+1][j+1] == 0) findZone(i+1,j+1,zoneNumber);
        if(i-1 >= 0 && j+1 < Columns && (notFix[i-1][j+1] || blackBound[i-1][j+1]) && zones[i-1][j+1] == 0) findZone(i-1,j+1,zoneNumber);
        if(i+1 < Rows && j-1 >=0 && (notFix[i+1][j-1] || blackBound[i+1][j-1]) && zones[i+1][j-1] == 0) findZone(i+1,j-1,zoneNumber);
        if(i-1 >= 0 && j-1 >= 0 && (notFix[i-1][j-1] || blackBound[i-1][j-1]) && zones[i-1][j-1] == 0) findZone(i-1,j-1,zoneNumber);
    }
    ...
    private void findend(int fx,int fy,int x,int y,boolean[][] found,boolean[][] road){
        if(road[x][y] || flag[x][y] != 1) return;
        if(blackBound[x][y] && found[x][y] == false && zones[fx][fy] == zones[x][y]){
            found[x][y] = true;
            solver.addAClause(path[fx][fy][x][y],path[x][y][fx][fy]);
        }
        if(x==0 || x==Rows-1 || y==0 || y==Columns-1)
            isBoundOfblackBound[fx][fy] = true;
        road[x][y] = true;
        if(x-1 >= 0 && y-1 >= 0) findend(fx,fy,x-1,y-1,found,road);
        if(x-1 >= 0 && y+1 < Columns) findend(fx,fy,x-1,y+1,found,road);
        if(x+1 < Rows && y-1 >= 0) findend(fx,fy,x+1,y-1,found,road);
        if(x+1 < Rows && y+1 < Columns) findend(fx,fy,x+1,y+1,found,road);
        road[x][y] = false;
    }
    ...
    protected void CnfRule3(){
        ...
        boolean[][] road = new boolean[Rows][Columns];
        for(i = 0; i < Rows; i++)
            for(j = 0; j < Columns; j++){
                found[i][j] = false;
                road[i][j] = false;
        ...
        for(i=0;i<Rows;i++)
            for(j=0;j<Columns;j++) if(zones[i][j] != 0){
                if(InMatrix(i+1,j+1) && zones[i+1][j+1] != 0)
                    if((i==0 || i==Rows-1 || j==0 || j==Columns-1 || isBoundOfblackBound[i][j]) && (i+1==0 || i+1==Rows-1 || j+1==0 || j+1==Columns-1 || isBoundOfblackBound[i+1][j+1])){
                        if(notFix[i][j] && notFix[i+1][j+1])  solver.addAClause(white[i][j],white[i+1][j+1]);
                        else if(notFix[i][j]) solver.addAClause(white[i][j]);
                        else if(notFix[i+1][j+1])  solver.addAClause(white[i+1][j+1]);
                    }
                    else if(i==0 || i==Rows-1 || j==0 || j==Columns-1 || isBoundOfblackBound[i][j]){
                    ...
                    else if(i+1==0 || i+1==Rows-1 || j+1==0 || j+1==Columns-1 || isBoundOfblackBound[i+1][j+1]){
        ...
        for(i=0;i<Rows;i++)
            for(j=0;j<Columns;j++)
                if (zones[i][j] != 0)
                    for(k=0;k<Rows;k++)
                        for(h=0;h<Columns;h++)//abs(i-k)=1 && abs(j-h)=1 => v(i,j) > v(k+1,h+1)
                            if(Diff(i,j,k,h) && zones[k][h] == zones[i][j])
                                if(Math.abs(i-k) == 1 && Math.abs(j-h) == 1){  
                                    if(InMatrix(k+1,h+1) && zones[k+1][h+1] == zones[i][j] && value(i,j) > value(k+1,h+1))
    ...
    protected boolean InsertAClause(){
        NumberOfClauses++;
        int[] arr = new int[nrOfCellNotFix];
        int i,j,len = 0;
        for (i=0;i<Rows;i++)
            for (j=0;j<Columns;j++)
                if(notFix[i][j]){
